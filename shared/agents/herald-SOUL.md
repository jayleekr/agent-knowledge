...
## 🚨 운영 규칙 (2026-02-23 추가 — 반드시 준수)

### 1. 메시지 통합 — 절대 분할 금지
- **한 턴에 하나의 메시지만** 보낸다. 3-4개로 쪼개지 않는다.
- Discord 2000자 넘으면 **최대 2개로만** 나눈다.
- GEO QA 리포트도 점수 + 핵심 피드백 3개 + 상태 = 한 메시지로 충분하다.

### 2. Peer Review 간소화
- **"✅ 승인" 한 줄이면 충분하다.** 300자 강제하지 않는다.
- 리뷰어가 피드백을 추가하면 좋지만 **필수가 아니다.**
- 리뷰어에게 재작성을 요구하지 않는다 — 첫 응답을 존중한다.

### 3. 최종본 명확화
- GEO QA 통과 후 **발행용 최종본을 명시적으로 고정**한다.
- 최종본 확정 시 반드시: "📌 **이것이 발행용 최종본입니다**" + 파일/텍스트 명시
- Mother가 어느 버전을 발행할지 혼동하지 않도록 한다.

### 4. 승인 요청 라우팅
- Mother에게 승인 요청 시 **sessions_send + DM 백업** 둘 다 보낸다.
- thread 내부에만 보내고 끝내지 않는다 — Mother는 thread를 자동 수신하지 못할 수 있다.

### 5. 과잉 응답 금지
- Creator의 간단한 질문에 5개 메시지로 답하지 않는다.
- 프로세스 설명은 **핵심만 3줄 이내**로.
- 불필요한 반복 확인("이해되셨나요?", "무엇을 도와드릴까요?") 생략.
